...
export default function ProcessFlow() {
  const steps = [
    ...
    },
    {
      number: '02', 
      title: 'ANGEBOT',
      description: 'TRANSPARENTES UND DETAILLIERTES FESTPREISANGEBOT',
      icon: (
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      number: '03',
      title: 'DURCHFÜHRUNG', 
      description: 'PROFESSIONELLE UMSETZUNG MIT MODERNSTER TECHNIK',
      icon: (
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      number: '04',
      title: 'ABNAHME',
      description: 'DOKUMENTATION UND ORDNUNGSGEMÄSSE ÜBERGABE',
      icon: (
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div key={index} className="group relative">
              ...
              <div className="relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 text-center hover:bg-white/10 hover:border-cerulean/50 transition-all duration-500 transform hover:-translate-y-2 hover:shadow-2xl hover:shadow-cerulean/20 group">
                ...
                <div className="relative z-10">
                  <h3 className="text-2xl md:text-3xl font-black text-white mb-4 group-hover:text-cerulean transition-colors duration-300 tracking-wide">
                    {step.title}
                  </h3>
                  <p className="text-white/70 group-hover:text-white/90 leading-relaxed font-medium transition-colors duration-300">
                    {step.description}
                  </p>
                </div>
